...
- **Fase corrente:** 4, Skin (SPEC §8/§2.3, `docs/PHASES.md`). Completata: griglia 2 colonne con contatore/attenuazione/ordinamento/filtro, dettaglio personaggio con switch di sblocco e checkbox outfit (gruppi di cibo mostrati, default sempre posseduto e non spuntabile), contatore Home aggiornato in tempo reale. Consigliami resta placeholder (fuori scope, fase 6).
- **Fase 3** (Seed nell'app): completata — entità Room definitive, `SeedRepository` con reseed, task Gradle `copySeedAssets`/`generateSeed`, Home collegata ai dati reali.
...
- **Aperto:** il percorso di rete reale di `generateSeed` non è stato eseguito end-to-end (l'ambiente di sviluppo non ha `python3-venv`/`ensurepip` di sistema, niente sudo) — verificato solo con un eseguibile Python finto al posto di seedgen, da riprovare su una macchina vera prima di una release reale. Nessun emulatore/dispositivo disponibile in questo ambiente: le schermate Skin non sono state verificate visivamente, solo build/lint/test JVM (Robolectric). Immagini dei personaggi: ancora nessuna alternativa originale ai render Nintendo, placeholder con iniziali confermato anche per Monete/Pulsanti P (fasi 5-6).
