...
- Risultati: schermata dedicata `ui/results/` (pulsante largo in Home + drawer), **unico** punto di registrazione. Tabella `best_results` con un solo `TrophyRank` per (evento, cilindrata), upsert. Nessun riporto tra cilindrate: un trofeo vale solo dove è registrato. Consigliami legge i risultati e basta.
...
- Nomi in italiano: `nameIt: String?` su `characters`/`outfits`/`courses` (mai su eventi/regioni/pulsanti P: nessuna fonte ufficiale, vedi Stato attuale), popolato da `tools/seedgen/seedgen/i18n.py`. `domain/model/localizedName(name, nameIt)` sceglie in base a `Locale.getDefault()`, chiamata nei ViewModel (non nei DAO, eccetto `PSwitchWithLocation.locationName` — una proprietà calcolata, stesso pattern di `CharacterProgress.isComplete`) o nelle Composable dove il ViewModel espone ancora l'entità grezza (Skin).

## Stato attuale
<!-- Aggiornare a OGNI fine sessione: fase corrente, cosa è fatto, cosa resta, problemi aperti. -->
- **Roadmap `SPEC.md` §8 completo, release 0.1.1 pubblicata dall'autore.** Dopo la 0.1.1 l'autore ha segnalato tre problemi (navigazione, buco di analisi Consigliami, nomi in inglese), tutti corretti; poi ha chiesto anche la doppia localizzazione dell'app. Riassunto di quest'ultimo giro:
  - **Nomi ufficiali in italiano**: `tools/seedgen/seedgen/i18n.py` estrae i nomi ufficiali (mai tradotti a mano) dalle tabelle "Names in other languages" di mariowiki per personaggi (24), corsi (30) e outfit alternativi (103, con varianti maschile/femminile dove l'italiano le distingue). **Nessuna fonte ufficiale verificabile** per missioni dei pulsanti P, regioni/biomi (nemmeno i nomi inglesi sono ufficiali lì) e Gran Premi/Knockout Tour: restano in inglese, con la fonte controllata e la lacuna documentata invece di inventare una traduzione. `domain/model/localizedName()` sceglie `nameIt` se l'app è in italiano ed esiste, altrimenti l'inglese — usato in Skin, Consigliami e nella colonna "corso" dei Pulsanti P.
  - **App localizzata IT/EN**: `values-en/strings.xml` + `locales_config.xml` (selettore lingua di sistema, Android 13+). Nessun selettore in-app (`AppCompatDelegate.setApplicationLocales()`): non richiesto, e avrebbe richiesto `AppCompatActivity`/dipendenza AppCompat, finora deliberatamente esclusa (vedi Decisioni prese, fase 1).
  - **Migrazione Room v1→v2** (`MIGRATION_1_2`, colonna `nameIt` su characters/outfits/courses): la 0.1.1 è già installata da utenti reali, quindi niente `fallbackToDestructiveMigration()` — cancellerebbe anche lo stato utente. Testata con `MigrationTestHelper` (Robolectric).
  - Correzione importante di questa sessione: **il percorso di rete reale di `generateSeed` FUNZIONA in questo ambiente** (`python3` di sistema ha già `requests`/`pyyaml`, non serve una venv) — la nota "mai eseguito end-to-end" nelle sessioni precedenti era sbagliata, era solo la venv dedicata di Gradle a non essere creabile (manca `python3-venv`/`ensurepip`, niente sudo). `seedVersion` è arrivato a 3 con dati reali, non con un eseguibile finto.
- **Risultati rivisti (post-0.1.2):** l'inserimento esce da Consigliami e va in una schermata dedicata (pulsante largo in Home + voce nel drawer). Il modello passa da stelle+posizione indipendenti a un unico trofeo (bronzo…oro ★★★), un valore per evento e cilindrata. Migrazione Room v2→v3 (`MIGRATION_2_3`) e backup v2 che rilegge ancora i v1. Verificato con build, lint e test JVM, **non a schermo**.
...
- **Aperto:** nessun emulatore/dispositivo Android disponibile in questo ambiente — le correzioni sono verificate solo con build/lint/test JVM (Robolectric), non visivamente; i nomi in italiano specificamente andrebbero controllati a schermo (i test coprono l'estrazione e il DAO, non il rendering). Immagini dei personaggi: ancora nessuna alternativa originale ai render Nintendo (placeholder con iniziali su tutte le schermate).
...
- 2026-09-25 · Nomi ufficiali in italiano: ricerca pagina per pagina su mariowiki prima di scrivere codice (vedi `aliases.yaml`, campo `wikiPage` sui corsi/il personaggio "peach"). Molti corsi ricorrono da giochi precedenti: il titolo senza disambiguatore è spesso una pagina di disambiguazione o una panoramica multi-gioco con la traduzione SBAGLIATA (storica, non quella di Mario Kart World) — mai fidarsi del primo risultato, verificare che la pagina parli davvero di Mario Kart World.
- 2026-09-25 · Sulle pagine "Names in other languages", la riga SENZA nota è quella attuale; righe successive con nota (nome di un vecchio gioco/media) sono varianti storiche — scoperto perché "Toad" in italiano ha anche "Ughetto" (nota "DIC cartoons"), preso per sbaglio con una logica "ultima riga = più recente" rivelatasi opposta a quella corretta.
- 2026-09-25 · Outfit con nota "Masculine"/"Feminine" sull'italiano (Explorer, Farmer, Sailor, Soft Server, Touring): variante scelta in base a `tools/seedgen/manual/character_genders.yaml`, tabella manuale (non estraibile dal wiki per personaggio) coerente con le localizzazioni italiane ufficiali della serie.
- 2026-09-25 · Le pagine per i nomi in altre lingue (una per personaggio/corso, fino a 2-3 MB l'una da biografie intere) **non sono fixture committate** come le altre pagine sorgente: gonfierebbero il repo di decine di MB per poche righe di tabella usate. Si scaricano sempre dal vivo (`wiki.fetch_i18n`), mai da `--from-fixtures`; testate con fixture sintetiche (`tests/test_i18n_synthetic.py`) più un modulo a rete live skippato senza connessione (`tests/test_i18n_live.py`).
...
- 2026-09-25 · Trovato e corretto un bug preesistente (non causato da questa sessione, riprodotto anche in CI su PR #19): `app/schemas/` (export dello schema Room) era gitignorata per intero. Su un checkout pulito, `MigrationTest` falliva con `FileNotFoundException` in modo intermittente perché KSP scrive lo schema come effetto collaterale non dichiarato come output del task — con la Gradle build cache ripristinata da un'altra esecuzione, `kspDebugKotlin` può risultare aggiornato senza aver mai scritto il file sul checkout corrente. **`app/schemas/` ora è versionata** (rimossa da `.gitignore`): è la soluzione raccomandata dalla documentazione ufficiale di Room, ed è l'unica corretta qui perché Room non riscrive gli schemi delle versioni precedenti a ogni build (li accumula nel tempo) — dichiararla come *output* di `kspDebugKotlin` (primo tentativo, scartato) fa sì che Gradle cancelli quelli non riscritti dall'ultima esecuzione, perdendo `1.json`. Aggiunta comunque una `dependsOn` esplicita `mergeDebugAssets` → `kspDebugKotlin` in `app/build.gradle.kts` per l'ordine nella stessa build. `1.json` (schema v1, pre-`nameIt`) ricostruito compilando temporaneamente con `version = 1` e le tre colonne `nameIt` rimosse dalle entity, poi tutto ripristinato: nessun dato di gioco toccato, solo uno schema derivato dal codice sorgente stesso.

- 2026-09-25 · Risultati: l'autore ha corretto il modello di fase 7. Stelle e posizione **non** sono indipendenti: la scala è bronzo < argento < oro < oro ★ < oro ★★ < oro ★★★ (confermato su Game Rant; per i KO TheGamer: 1° oro, 2° argento, 3° bronzo, dal 4° nessun trofeo). `improvement = 1 - level/6`, generalizzazione lineare della vecchia `1 - stelle/3`: scelta di prodotto, non un dato di gioco. Si salva solo il migliore (scelta dell'autore): un nuovo trofeo sostituisce il precedente.
- 2026-09-25 · Nessun riporto tra cilindrate: una fonte (Kotaku) dice che un risultato a cilindrata alta vale anche per le inferiori, ma l'autore non ne è sicuro e ha chiesto di non applicarlo. `bestRank(E, cc)` usa solo il trofeo registrato a cc. La migrazione v2→v3 converte, non cancella: 1° → oro + stelle, 2° argento, 3° bronzo, il resto nessuna riga. `characterId`/`timestamp` del vecchio storico si perdono (nessun uso nel nuovo modello).
