...
- Risultati: schermata dedicata `ui/results/` (pulsante largo in Home + drawer), **unico** punto di registrazione. Tabella `best_results` con un solo `TrophyRank` per (evento, cilindrata), upsert. Il riporto verso le cilindrate inferiori è `domain/model/effectiveRank()`, applicato nei ViewModel, mai in SQL. Consigliami legge i risultati e basta.
...
- 2026-09-25 · Un trofeo vale anche per le cilindrate inferiori e lo Specchio per tutte (Kotaku), quindi `bestRank(E, cc)` guarda cc e superiori. La migrazione v2→v3 converte, non cancella: 1° → oro + stelle, 2° argento, 3° bronzo, il resto nessuna riga. `characterId`/`timestamp` del vecchio storico si perdono (nessun uso nel nuovo modello).
